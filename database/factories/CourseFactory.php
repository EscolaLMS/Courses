...
class CourseFactory extends Factory
{
    ...
    public function configure()
    {
        return $this->afterMaking(function (Course $course) {
            ...
            $filename_image = "courses/$id/".$word.'.jpg';
            $filename_video = "courses/$id/".$word.'.mp4';
            $filename_poster = "courses/$id/".$word.'poster.jpg';

            //copy(realpath(__DIR__.'/../mocks/1.pdf'), Storage::path($path.$filename));

            $dest_image = Storage::path($filename_image);
            $dest_video = Storage::path($filename_video);
            $dest_poster = Storage::path($filename_poster);
